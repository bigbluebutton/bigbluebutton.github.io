...
```
Next, add the BigBlueButton repository URL (we retain the trusty-100 for the latest release 1.0.1 to make it easier for everyone to upgrade their server).

```
$ echo "deb http://ubuntu.bigbluebutton.org/trusty-100/ bigbluebutton-trusty main" | sudo tee /etc/apt/sources.list.d/bigbluebutton.list
```
