...
Want to learn more about BigBlueButton?  The documentation site below gives you all the details on how to install, configure, and customize BigBlueButton to your needs.


## BigBlueButton overview

BigBlueButton is an HTML5-based web application.  Unlike many commercial web conferencing systems that require you to install software, BigBlueButton runs within your web browser.  There is no binary to download, no plugin to install.
...
BigBlueButton has a "mobile first" design.  The product is designed to run on a mobile device. 

Like the desktop, there is no mobile app to download or install on your phone and tablet.  BigBlueButton runs within your mobile browser.  
...
It's worth emphasizing that there isn't a mobile client and a desktop client -- the same HTML5 client runs on all devices.  This gives us a single code base to enhance and innovate with adoption across desktop, laptop, chromebook, iOS, and Android.  Nice!
...
There are two types of users in BigBlueButton: viewer and moderator.  
  ...
  * A _viewer_ (typically the student) can chat, send/receive audio and video, respond to polls, display an emoji (such as their raised hand), and participate in a break out room. 

  * A _moderator_ (typically the instructor) has all the capabilities of a viewer plus the ability to mute/unmute other viewers, lock down viewers (such as restrict them from using private chat), and assign anyone (including themselves) the role of _presenter_.  
...
To show users how to use the above features, we created two overview videos: one for the viewer and the other for the moderator:
...
1. Setup your own BigBlueButton server in 15 minutes using [bbb-install.sh](https://github.com/bigbluebutton/bbb-install), or follow our [step-by-step install instructions](/2.2/install.html).
