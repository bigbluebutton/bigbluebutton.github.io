...
Welcome to the installation guide for BigBlueButton version 1.0-beta (referred hereaver as BigBlueButton 1.0).  BigBlueButton is an open source web conferencing system for on-line learning.  For an overview what new in this release, see [overview](/overview/10overview.html).
...
```
Next, add the BigBlueButton repository URL (Note the 'xxx' will soon be updated).

```
$ echo "deb http://ubuntu.bigbluebutton.org/xxx/ bigbluebutton-trusty main" | sudo tee /etc/apt/sources.list.d/bigbluebutton.list
```
...
If already have a BigBlueButton 0.81 server, these instructions will **not** upgrade your server.  To install BigBlueButton 0.9.1, you need to install on a clean installation of Ubuntu 14.04 64-bit and follow the steps previous in this document.
