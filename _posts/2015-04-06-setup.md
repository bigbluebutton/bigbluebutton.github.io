...
Before you can start developing on BigBlueButton, you to [install BigBlueButton 0.9.0](/setup/install.html) and ensure it's working correctly (i.e. you had no errors during install).
...
Next you need to get the Apache Flex 4.14.1 SDK package. 

**Note:** Even though we're downloading the Apache Flex 4.14.1 SDK, BigBlueButton is developed and built with Flex 3 compatibility mode enabled.

First, you need to download the SDK tarball from an Apache mirror site and then unpack it.

```
wget http://apache.mirror.gtcomm.net/flex/4.14.1/binaries/apache-flex-sdk-4.14.1-bin.tar.gz
tar xvfz apache-flex-sdk-4.14.1-bin.tar.gz
```

The Flex SDK is now unpacked.  Next you need to download the Adobe Flex SDK.  We'll do this step manually incase the download fails (if it does, remove the incomplete file and issue the `wget` command again).

```
cd apache-flex-sdk-4.14.1-bin/
mkdir -p in/
wget http://download.macromedia.com/pub/flex/sdk/builds/flex4.6/flex_sdk_4.6.0.23201B.zip -P in/
```

Once the SDK has downloaded we can use the SDK's `build.xml` script to automatically download the remainin third-party tools.
...
```
sudo find ~/dev/tools/apache-flex-sdk-4.14.1-bin -type d -exec chmod o+rx '{}' \;
chmod 755 ~/dev/tools/apache-flex-sdk-4.14.1-bin/bin/*
sudo chmod -R +r ~/dev/tools/apache-flex-sdk-4.14.1-bin
```

Next, create a linked directory with a shortened name for easier referencing.

```
ln -s ~/dev/tools/apache-flex-sdk-4.14.1-bin ~/dev/tools/flex
```
...
```
cd ~/dev/tools/
mkdir -p apache-flex-sdk-4.14.1-bin/frameworks/libs/player/11.2
cd apache-flex-sdk-4.14.1-bin/frameworks/libs/player/11.2
...
```
cd ~/dev/tools/apache-flex-sdk-4.14.1-bin
...
We're currently developing 0.9.0 on master. When we release 0.9.0 final, we'll update these instructions to checkout a branch related to 0.9.0 development.  For now, you'll want to be on master.
...
The first thing we need to do is to copy the latest config.xml file and update it to match our installation.
...
```
# Update config.xml (replace the 192.168.1.145 with your BigBlueButton hostname/ip address)
...
The config.xml file tells the BigBlueButton client what modules to retrieve and how to connect to the server. Later on, when you deploy your modified client to the BigBlueButton server, there will be two clients: your modified client and the default packaged client (again, this is good as you can switch back and forth). However, the BigBlueButton configuration command `sudo bbb-conf ` only modifies the packaged client. If you need to change your config.xml (such as when the IP address of your server changes), you need to do it manually.

If the above does not make sense -- i.e. that there are two `config.xml` files and `bbb-conf` only changes the packaged `config.xml` (not your copy in the dev folder), then when you run your development copy of the client it may not finish loading.  You'll see the words `Connecting ....`.  You need to ensure that the config.xml has the correct IP address for your BigBlueButton server.  You can fix this with the above sed command.
...
Note: You could have also restarte diwht `sudo bbb-conf --restart`, but it's a good idea to pass `--clean` instead as it cleans out all the log files between restart and thereby wipes and previous errors.
