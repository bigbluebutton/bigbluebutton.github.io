...
```
Next, get the latest version of the install script using the following command:

```bash
wget https://raw.githubusercontent.com/bigbluebutton/greenlight/v3/greenlight-install.sh
```

To use the install script, you must have the BigBlueButton Endpoint and Secret of the server that you want Greenlight to connect to. If you do no have those values, you can retrieve them by running `bbb-conf --secret` on your BigBlueButton server.

Once you have those values, you can run the install script (making sure you replace <BIGBLUEBUTTON_ENDPOINT> <BIGBLUEBUTTON_SECRET>)
```bash
bash greenlight-install.sh <BIGBLUEBUTTON_ENDPOINT> <BIGBLUEBUTTON_SECRET>
```
