...
For the current version of Alpha, you'll need to manually create your Administrator. To do that, you must first sign up with the account that you would like to promote to an administrator. Once you've done that, there are 2 commands that you need to run: 
```bash
docker exec -it greenlight bin/rails c 
User.find_by(email: <ADMINISTRATOR EMAIL>).update(role: Role.second)
```
