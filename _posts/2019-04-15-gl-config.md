...
Second, edit your `docker-compose.yml` to include the PostgreSQL container (You can use vi, vim, nano or any text editor)
```bash
vim docker-compose.yml
```
Remove the `#` before every line below the `# When using postgresql as the database` line (Making sure to replace the **RANDOM_PASSWORD_REPLACE_ME**). Make sure you keep the spacing consistent. What you should end up with is:
```yaml
    links:
      - db
  db:
    ...
    environment:
      ...
      - PGDATABASE=postgres
      - PGUSER=postgres
      - PGPASSWORD=RANDOM_PASSWORD_REPLACE_ME
```
In the uncommented text, you'll also want to replace
```yaml
  - ./db/production:/var/lib/postgresql/data
```
With:
