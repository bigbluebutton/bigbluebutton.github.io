...
Note: These instructions depend on a `ffmpeg` repository that is currently offline.  As a result, the install will not finish.  We're in the process of updating the `ffmpeg` repository for BigBlueButton.   Check back shortly and, when this message is gone, you should be able to install without error.
