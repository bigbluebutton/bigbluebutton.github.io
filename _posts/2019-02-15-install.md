...
Welcome! This document is the starting point for installing BigBlueButton 2.2-beta (referred hereafter as BigBlueButton).
...
BigBlueButton has a very active open source community.  There are over 2,500+ member of the developer mailing list. 

For questions related to installation and setup, first read through this guide, check out the troubleshooting section, and do a quick Google search for your error message.  If you don't find a resolution to your issue, make post to the [bigbluebutton-setup mailing list](https://groups.google.com/forum/#!forum/bigbluebutton-setup) with the following information

  * What is the installation issue you are encountering (post any error messages you are seeing)?
...
All the core developers hang out in bigbluebutton-setup and try to help out newcomers.  If there is a genuine issue with BigBlueButton's installation, we want to know about it and fix it ASAP.

When engaging the community, providing this information will make it *much* easier for a someone to volunteer their time to help you out.  In contrast, if you simply posted: 

>  My redis server status is not woring -- please help me to overcome this issue. 

there isn't much go to on.  A community member would need to post back asking a series of questions to figure out, for example, that there was an error message given during installation.

A common source of issues is trying to install BigBlueButton on a server that is underpowered or already running other web applications (that will confict with BigBlueButton in subtile ways).  Read the [Before you install](#before-you-install) next.

# Before you install

BigBlueButton installation only works if your server meets the minimum requirements for running BigBlueButton.  

We recommend installing BigBlueButton on a 'clean' Ubuntu 16.04 64-bit server.  By 'clean' we mean the server should not have any previous web applications installed (such as plesk, webadmin, or apache).  Such web applications want to bind to port 80 and will conflict with BigBlueButton.
...
Got a Ubuntu 16.04 64-bit server setup?  Great!  But before jumping into the installation, let's do a few quick configuration checks to make sure the server does meet the minimum requirements.  This will prevent you from encountering errors later in the installation.
