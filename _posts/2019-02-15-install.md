...
The HTML5 client runs without plugins.  This means no Flash, no Java, and no downloadaded pluging to install.   It's a pure HTML5 client that takes advantage of the latest capabilities of FireFox and Chrome (the recommended browsers).
...
If you want to set up a BigBlueButton server quickly (or have already setup BigBlueButton servers in the past), use [bbb-install.sh](https://github.com/bigbluebutton/bbb-install) to set up your server with a single command.  
...
If you are doing a new install of BigBlueButton, skip to [Installation](#Installation).
