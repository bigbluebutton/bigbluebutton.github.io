...
Welcome to the install guide for BigBlueButton 2.2-beta (referred hereafter as BigBlueButton).
...
For the end user, BigBlueButton offers real-time sharing of audio, video, presentations, and screen.  To engage students, BigBlueButton offers interactive chat (public and private), multi-user whiteboard, shared notes, emojis, polling, and breakout rooms.  BigBlueButton records all content for later playback.

BigBlueButton is not your average web application.  In addition to handling real-time streams of media and data between clients, the BigBlueButton server runs a number of back-end processes to handle incoming [API calls](/dev/api.html), processing of uploaded slides, and converation of captured sessions into [recordings](/dev/recording.html).

## HTML5 client
This release of BigBlueButton puts the HTML5 client in the forefront.  While the Flash client is still available, this release marks the first time you can use the HTML5 client as the default client.

This means the HTML5 client now covers the main functionalities for both viewers and moderators, as well as the presenter role.  The HTML5 client runs without plugins.  No Flash, no Java, and no mobile app needed to install.    It's a pure HTML5 client that takes advantage of the latest capabilities of FireFox and Chrome (the recommended browsers), but also works on Safari and Edge (with some limitations).

The benefits of the HTML5 client include:
  * 2x faster loading than the Flash client
  * WebRTC for higher-quality audio, video, and screen sharing 
  * Robust shared notes for mult-user editing (built using the excellent [EtherPad](https://etherpad.org/) project)
  * Full accessible to screen readers
  * Presenters can share YouTube videos during the session

We've been steadily re-implementing the remaining features of BigBlueButton from the Flash client.  There are a few capabilities left to implement
...
Setting up a BigBlueButton server 

When installing BigBlueButton you have two choices: 
  * Scripted install with [bbb-install.sh](https://github.com/bigbluebutton/bbb-install) using `-v xenial-220-beta` (not `-v xenial-200`) to install BigBlueButton 2.2-beta
  * Step-by-step instructions (this guide)

If you want to setup a BigBlueButton server quickly (or have already setup BigBlueButton servers in the past), `bbb-install.sh` will get you going with a single command.  If you want to understand more of the components of BigBlueButton, do the step-by-step install.

Regardless of which choice you make, you still need to prepare for a successful install by
  * obtaining a dedicated server,
  * making sure the server meets BigBlueButton's [minimum set of requirements](#minimum-server-requirements), 
  * assigning a hostname (recommended to setup SSL), and
...
Note 2: If the installation exits with an error before finishing, doulbe-check the steps in [Before you install](#before-you-install).  If you find an resolve any configuration errors, you can attempt to finish the installation using the command `sudo apt-get install -f`.

Note 3: If you don't get a clean install and `sudo apt-get install -f` still gives you errors, stop here.  BigBlueButton didn't finish installing and will not run.  See the troubleshooting guide and other options for [getting help](#getting-help).

After the installation finishes, if you want to make the HTML5 client the default client (recommended), edit `/usr/share/bbb-web/webapps/bigbluebutton/WEB-INF/classes/bigbluebutton.properties` and set both `attendeesJoinViaHTML5Client` and `moderatorsJoinViaHTML5Client` to `true`, as in
...
With the installation finished, restart BigBlueButton's components with the following command:

~~~
$ sudo bbb-conf --restart
~~~

Note: Don't worry if you initially see `# Not running:  tomcat7 or grails` or `Error: Could not connect to the configured hostname/IP address` as the startup takes a few moments.

Next, check the setup using `bbb-conf --check`.  When you run this command, you should see output similar to the following:
...
However, you can't login from this screen unless you install the API demos (see next step).

You can integrate BigBlueButton with one of the 3rd party integrations by providing the integration the server's addres and shared secret.  You can use `bbb-conf` to dispaly this information using `bbb-conf --secret`.
...
If this server is intended for production, you should
  ...
  * [Setup a TURN server](/2.2/setup-turn-server.html) to enable users behind restrictive firewalls to send/receive media using WebRTC
  * [remove the API demos](/2.2/customize.html#remove-the-api-demos)

We provide to servers that you can use for testing:
