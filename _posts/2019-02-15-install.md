...
title: "Install"
category: 2.2
redirect_from: "/install/install.html"
redirect_from: "/2.0/20install.html"
redirect_from: "/2.0/20overview.html"
redirect_from: "/2.2/overview.html"
redirect_from: "/overview/architecture.html"
...
BigBlueButton has very detailed documentation (this guide), videos, and the following [mailing lists](https://bigbluebutton.org/support/community/):

   * bigbluebutton-setup - Installation/configuration questions
   * bigbluebutton-dev - Developer questions
   * bigbluebutton-users - End user questions 

For example, the bigbluebutton-setup mailing list alone has over 2,500+ members.  When applying to join one of the mailing lists, be sure to answer the challenge question (otherwise your application will look like a SPAM bot trying to join and will be denied).

Your first step in getting help is doing Google search.  Google indexes all our documentation, GitHub issues, and mailing lists.  If you have encountered a known issue, there is an excellent chance that others have done so as well, and Google will find for you the existing documentation, issue, or forum post that references it.

If a bit of googling does not lead you to an existing reference, then consult the associated section below.

### I have a problem using BigBlueButton

If you are an end-user and you have encountered an issue with using BigBlueButton itself, first check the following tutorial videos and frequently asked questions (FAQ):
  ...
  * Check the [FAQ](http://docs.bigbluebutton.org/support/faq.html)

If you don't see a solution, post to [bigbluebutton-users](https://groups.google.com/forum/#!forum/bigbluebutton-users) and describe your problem.  Include the following information
 
   1. Steps to reproduce the problem
   1. A screen shot of the problem (or error message)
   1. Let us know if you can reproduce the problem on our [demo server](https://demo.bigbluebutton.org)?

### I have a problem installing BigBlueButton

For installation/configuration problems, double-check that your server meets the [minimum requirements](/2.2/install.html#minimum-server-requirements).  If your server does not, then the environment constraints are likely causing your server to fail in unpredictable ways.  Stop before going further and install BigBlueButton on a server that does meet (or exceed) the requirements.

Next, if your server has a public IP address and a fully qualified domain name (FQDN), try installing/upgrading using the [bbb-install.sh](https://github.com/bigbluebutton/bbb-install) script.  This script automates much of the [step-by-step](/2.2/install.html#before-you-install) installations (though it's still good to read through these steps to know what the script is doing).

Next, checkout the following doc
  1. Installation [troubleshooting guide](/2.2/troubleshooting.html) (this has a *lot* of troubleshooting tips)
  2. [FAQ](http://docs.bigbluebutton.org/support/faq.html)
  1. For recording-related issues, see [Recording troubleshooting section](/dev/recording.html#troubleshooting)

Next, check the output of `sudo bbb-conf --check`, which will check the configuration of your server and, if it finds issues, will post recommendations on how to resolve.

If after doing the preceding steps you are unable to solve the problem, then post as much information as you can about the problem to [bigbluebutton-setup](https://groups.google.com/forum/#!forum/bigbluebutton-setup).

BEFORE YOU POST, we emphasize the advantage to you to post as much information as possible.  Consider this post

> Help, my BigBlueButton server stopped working.

There is no information about the server, no description of the problem, and, most importantly, no steps to reproduce the problem.  

In contrast, when you post, *provide as much information as possible* about your problem to help others diagnose and narrow down the root cause.

Here is a checklist of basic information to include in the post:

  1.  What version of BigBlueButton are you running?
  ...
  1.  Did the problem appear after initial install, after an upgrade, or after running the server for a while?
  ...
  1.  Are you running BigBlueButton from packages or from a within a development environment?
  1.  Post the output of `sudo bbb-conf --check`

For any issues that may be network related -- such as problems sharing audio or video -- also include 
...
For any issues that may be client releated -- such as encountering an error when using the client -- also include 

  1. Are you able to reproduce the issue on [https://demo.bigbluebutton.org/](https://demo.bigbluebutton.org/)?
  1. Are there any errors in the browser's console (if so, post the full browser log using [pastebin](https://pastebin.com/)?

Include as much information as you can, and it will make it easy for others to respond and help you get going quickly.


### I have a problem developing BigBlueButton

Post as much information as you can about your problem to [bigbluebutton-dev](https://groups.google.com/forum/#!forum/bigbluebutton-dev).

For development-related questions, the more you demonstrate you have first invested your time to solve the problem, the easier it is for others to volunteer their time to help.
...
If you installed a previous version of BigBlueButton 2.0 using the [2.0 installation](/install/install.html) and then made custom changes, you need to (you guess it!) backup your changes first.  After you backup your changes, can then use [bbb-install.sh](https://github.com/bigbluebutton/bbb-install) to upgrade or follow the steps below.  
...
Also, if your BigBlueButton server is behind a firewall, you may need to speciify the `value` with an external IP address `EXTERNAL_IP_ADDRESS:7443` to avoid getting an error 1002 in the client.  For more details see [Configure BigBlueButton behind a firewall](/2.2/configure-firewall.html).
