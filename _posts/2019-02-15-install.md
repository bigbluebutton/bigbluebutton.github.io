...
Encountered an issue with BigBlueButton?  No problem!  

BigBlueButton has very detailed documentation, videos, and an active open source community where members volunteer their time to help others (we were all beginners at some point).  For example, the bigbluebutton-setup mailing list alone has over 2,500+ users.  

The following sections provide help for solving three types of question:
...
Before you post, do a quick Google search of the problem or error message.

Google indexes all the documentation and [BigBlueButton mailing lists](https://bigbluebutton.org/support/community/), so, unless you've hit a new issue, there is an excellent chance that others have already encountered and there is a discussion or documentation that references it.

To join a mailing list, see [commiunity support](https://bigbluebutton.org/support/community/).  When applying to join, be sure to answer the challenge question otherwise we'll think your a SPAM bot and deny your application.

If you still have problems, check the associated section below.

### I have a problem using BigBlueButton

If your an end-user and have encountered an issue with using BigBlueButton itself, check the following tutorial videos.
...
If you don't see a solution, post to [bigbluebutton-users](https://groups.google.com/forum/#!forum/bigbluebutton-users) and describe your problem.  Include the following information
   ...
   * A screen shot of the problem (or error message)
   * Let us know if you can reproduce the problem on our [demo server](https://demo.bigbluebutton.org)?

### I have a problem installing BigBlueButton

For installation/configuration problems, check 

  1. Check out the [troubleshooting guide](/2.2/troubleshooting.html)
  2. Check out the [FAQ](http://docs.bigbluebutton.org/support/faq.html)

Next, read through the rest of this guide (it's easy to skip a step if you are following the step-by-step installation instructions.  In particular, ensure your server meets the [minimum requirements](/2.2/install.html#minimum-server-requirements). 

If your server has a public IP address and a fully qualified domain name (FQDN), try installing/upgrading using the [bbb-install.sh](https://github.com/bigbluebutton/bbb-install) script.

If the problem persists, post to the [bigbluebutton-setup](https://groups.google.com/forum/#!forum/bigbluebutton-setup) mailing list and provide enough information for others to help.

For example, this post is not very good.

  Help, my BigBlueButton server stopped working.

Imagine trying to help this user.  The user posted no information on the server (i.e. what version they are running), no information on the problem, and, least of all, no informtion on how to reproduce it.  This is also not a good post because it requires others to ask many question before they can start to help.

In contrast, a post that provides all the relevant information is much easier to respond.  When you post, 

Here's is a list of questions that you should provide to get help from others on installation/configuration issues.

  1.  What version of BigBlueButton are you running?
  1.  Did you get any errors during the install (and if so what were the errors)?
  1.  How did you [install BigBlueButton](/2.2/install.html#installation-choices) (step-by-step or bbb-install.sh)?
  1.  Did you make any custom changes to BigBlueButton outside using [bbb-conf](/admin/bbb-conf.html) or [bbb-install.sh](/2.2/install.html#bbb-installsh) script?
...
For any network-related issues (suchy as sharing audio or video), also include

  1.  Are you installing BigBlueButton on (a) a private network with no internet access, (b) a private network with internet access and a firewall, (c) a server on the internet with public/private IP addresses (such as Amazon EC2 or Azure), or (d) a server on the internet with a public IP address?
  1.  Are you installing on a dedicated server or virtual machine?
...
And finally
  ...
  1. What is the output of `sudo bbb-conf --check`?


If you can provide step-by-step instructions on how to reproduce the issue on the demo server, that will quickly get the attention of the core developers and give your issue a higher priority to resolve over others.

### I have a problem developing BigBlueButton

Review the development docs again.  If your trying to add a feature to BigBlueButton and encounter an issue, post specific information on the issue.
