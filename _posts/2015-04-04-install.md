...
Welcome to the installation guide for BigBlueButton 0.9.0-beta (referred hereafter as 0.9.0).  BigBlueButton is an open source web conferencing system for on-line learning.

For an overview what new in this release, see [overview](/overview/090overview.html).

If you already have a BigBlueButton 0.81 server, these instructions will **not** upgrade your server.  To install BigBlueButton 0.9.0, you need to install on a clean installation of Ubuntu 14.04 64-bit.  The reason is that BigBlueButton ran on Ubuntu 10.04 64-bit, whereas BigBlueButton 0.9.0 runs on Ubuntu 14.04 64-bit.  We recommend setting up a new Ubuntu 14.04 64-bit server and [copying over the recordings](#reprocess-recordings-from-0.81) from the old BigBlueButton server to the new.
...
And check your version of Ubuntu is Ubuntu 14.04.  If you try to install BigBlueButton 0.9.0 on any other release, it won't work.
...
If you've already installed an earlier build, see [upgrading BigBlueButton 0.9.0](#upgrading-bigbluebutton-0.9.0-beta)
...
## 4.  Install ffmpeg
BigBlueButton 0.9.0 uses ffmpeg to process recordings of a session for playback.
...
Please see [090TransferRecordings](#reprocess-recordings-from-0.81)
...
First, add the package repository for LibreOffice 4.3.  You only need to do this once and can skip this step in applying future updates to BigBlueButton 0.9.0.
...
To upgrade earlier versions of BigBlueButton 0.9.0, do the following:
...
The following lists the possible webRTC error messages:
...
This is the recommended way of copying recordings, since the recordings will be rebuilt using newer versions of the recording software, enabling new features and fixing bugs that may have been present with the 
old version. The downside is that this can take a long time, and will use a lot of CPU on your new BigBlueButton server while you wait for the recordings to process.
...
There are other ways of transferring these files; for example, you could create a tar archive of the `/var/bigbluebutton/recording/raw` directory, and copy it with scp, or use a shared NFS mount. Any method sho
uld work fine.
...
First, you need to transfer the contents of the `/var/bigbluebutton/published` and `/var/bigbluebutton/unpublished` directories. In addition, to preserve the backup of the original raw media, you should transfe
r the contents of the `/var/bigbluebutton/recording/raw` directory.
...
If you are transferring recordings from a BigBlueButton 0.80 server, they may have been done in an older recording format called "slides". In order to correctly play back these recordings on a new BigBlueButton
 server, you will have to install an additional package:
