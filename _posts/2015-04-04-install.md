...
Welcome to the installation guide for BigBlueButton 0.9.0-beta (referred hereafter as 0.9.0).  BigBlueButton is an open source web conferencing system for on-line learning.  For an overview what new in this release, see [overview](/overview/090overview.html).

We recommend you install BigBlueButton 0.9.0-beta on a clean Ubuntu 14.04 64-bit server that is not running any other web applications.  This will avoid subtle conflicts (such as with underlying configuration files) that will cause errors in BigBlueButton installation and running.
...
```
We have not tested trying to install BigBlueButton 0.9.0 on any other Ubuntu release (and it likely won't work)


# Installing BigBlueButton 0.9.0

If you've already installed an earlier build of 0.9.0, see [upgrading BigBlueButton 0.9.0](#upgrading-bigbluebutton-0.9.0-beta)

A few words about the installation steps below.  You must completely finish each step before proceeding to the next.  If you encounter an error at any step, STOP and double-check you've entered the proper commands.  Don't continue onto the next step (you'll only compound the errors).  If you can't resolve the error (use Google to search for the error messages), then post a description of the error to the [bigbluebutton setup](https://groups.google.com/forum/#!forum/bigbluebutton-setup) mailing list and the community will help.

Commerical support is available please see [support options](http://bigbluebutton.org/support).
...
## 4.  Install ffmpeg
BigBlueButton 0.9.0 uses ffmpeg to create compoents of the playback files (the audio, video, and desktop sharaing components).
...
If BigBlueButton didn't install or run, see [troubleshooting installation](#troubleshooting).
