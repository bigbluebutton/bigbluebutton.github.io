...
Welcome to the installation guide for BigBlueButton version 0.9.1 (hereafter referred to as 0.9).  BigBlueButton is an open source web conferencing system for on-line learning.  For an overview what new in this release, see [overview](/overview/090overview.html).

We recommend you install BigBlueButton 0.9 on a dedicated Ubuntu 14.04 64-bit server that is not running any other web applications.  This will avoid subtle conflicts (such as with underlying configuration files) that will cause errors in BigBlueButton installation and running.
...
The prerequisites for BigBlueButton 0.9 server are
...
```
We have designed, developed, installed, and tested BigBlueButton 0.9 on Ubuntu 14.04 64-bit (Trusty Tahr).  In other words, we have neither installed nor tested testedBigBlueButton on a different release of Ubuntu (and it will likely not work).
...
```
Next, add the BigBlueButton repository URL (we retain the trusty-090 for the latest release 0.9 to make it easier for everyone to upgrade their server).
...
## 4.  Install ffmpeg
BigBlueButton 0.9 uses ffmpeg to create componetns of the playback files (the audio, video, and desktop sharing components).
...
We're now ready to install BigBlueButton 0.9. Type
...
To upgrade your BigBlueButton 0.9 server, add the package repository for LibreOffice 4.3 (if you've not already done this).  To check if you have libreoffice 4.3, enter the command `dpkg -l | grep libreoffice` and you should see a version 4.3.X of libreoffice.
