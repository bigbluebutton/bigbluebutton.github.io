...
You need to make sure that the dialplan will accept the incoming secure websocket. Open the file `/opt/freeswitch/conf/dialplan/public/bbb_webrtc.xml` for editing and change:
<pre><code>&lt;condition field="${sip_via_protocol}" expression="^ws$" break="on-false"&gt;</code></pre>
To
<pre><code>&lt;condition field="${sip_via_protocol}" expression="^wss?$" break="on-false"&gt;</code></pre>
