...
After the planning phase, each feature for the release is assigned an issue in the BigBlueButton Issue Tracker (if it does not already have one).  This allows the community to track the progress of each release.  For example, to track the progress for 0.9.0, see [list of issues tagged for 0.81](https://code.google.com/p/bigbluebutton/issues/list?can=1&q=milestone%3DRelease0.9.0).
