...
The developer mailing list has over 2000 users, so before you post:
  ...
  1. If you think you've found a bug, first check the [issues database](https://github.com/bigbluebutton/bigbluebutton/issues) to check if it's already been reported.
...
Yes.  Canonical provides official Ubuntu EC2 AMIs for Ubuntu 14.04 64-bit.  To install BigBlueButton on EC2, do the following:
  ...
  1. Modify the default security group in (or create a new security group) so you have port 80, 9123, and 1935 open (see [FAQ](/support/faq.html#what-ports-must-be-open-for-external-users-to-connect-to-bigbluebutton)).
  1. Launch a 64-bit "trusty" instance from Canonical's [list of supported AMI](http://cloud.ubuntu.com/ami/).  We recommend running BigBlueButton on a c3.xlarge (or greater CPU) instance.
...
This work is underway.  See [HTML5 Client](/html/html5-overview.html) and [presentations from Developer Summit IX](https://youtu.be/Yuwbg_rz19o).

### iOS

This work is underway.  See [presentations from Developer Summit IX](https://youtu.be/qtPQE7YP1Ac).
...
### How do to receive calls from Internet Telephone Service Providers

Configure the server to receive calls from your SIP trunk provider. 

Find you provider in [Internet Telephone Service Providers](https://freeswitch.org/confluence/display/FREESWITCH/Providers+ITSPs) and configure FreeSWITCH accordingly.

Then, create a dialplan file in ```/opt/freeswitch/conf/dialplan/public/my_provider.xml``` with the following contents.

~~~xml
   <extension name="from_my_provider">
     <condition field="destination_number" expression="6135551234">
...
Make sure you change 6135551234 with your dial-in number.

Change ownership of the file to ```freeswitch.daemon```

```
chown freeswitch.daemon my_provider.xml
```
...
If you have [configured HTTPS](/install/install.html#configuring-https-on-bigbluebutton) on your BigBlueButton server, then you will need to also forward the following additional ports
...
There some open issues for improving the desktop sharing, see [1895](https://github.com/bigbluebutton/bigbluebutton/issues/1895).
...
For a detailed breakdown of the items in (1) see our [road map](/support/road-map.html).  If the feature you are requesting is not on our road map, you could open a [new issue](https://github.com/bigbluebutton/bigbluebutton/issues) and make a convincing argument that the feature belongs in (1).
...
Most importantly for everyone, the efforts to support a [white labeled](http://en.wikipedia.org/wiki/White-label_product) version of BigBlueButton won't take away from core development, but rather support it.
