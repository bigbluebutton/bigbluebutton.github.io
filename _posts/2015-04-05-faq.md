...
All of the core BigBlueButton contributors subscribe to all three mailing lists.  Please don't cross post to more than one list -- your only causing more effort to answer all the threads.
...
There is a very active BigBlueButton community of members on the developer mailing list (over 1800 members and counting!).  In the BigBlueButto community at-large all the members, users, developers, educational institutions, and commercial companies are all collaborating together on using and improving BigBlueButton.
...
It cannot be overstated that the project's focus is primarily on stability.  For a university or college to deploy BigBlueButton for live classes, or for a commercial company to embed BigBlueButton into their product, the software must be extremely stable.  To that end, you'll from the previous release notes that we tend to spend *months* testing each release before issuing a release candidate.
...
Like many open source projects, at the core of the project are at team of developers that have responsibility for core development and overall quality of the project.  The currenet committers are as follows:
...
The committers have earned this responsibility through years of conribution to BigBlueButton and to related open source projects (i.e. red5).  In particular, we very much respect Richard's seven year-plus effort to create BigBlueButton.  As the Lead Architect for our project, he has the final say.

The committers are very active in the support and mentoring of other developers in the [bigbluebutton-dev](http://groups.google.com/group/bigbluebutton-dev/topics?gvc=2) mailing list.  The BigBlueButton projet also participated in the 2010 Google Summer of Code (Google paid for two students to work on the project).
...
Each commit is reviewd by another committer who is familiar as well with the area of the product.  Any substantive commit to the core is reviewd by Richard Alam, BigBlueButton's CTO.
...
Once an update has been committed, it is tested by other developers in the latest build.  In the way, a build will itterate through development towards beta.  The release stays in development until all core development is finishe and all obivous bugs have been fixed.
...
Once a release is moved to beta, the public Ubuntu packages are updated so others in the BigBlueButton can begin installing and using the build.
...
You'll need good upstream and downstream bandwidth from the server.  We recommend (at least) 100 Mbit/second bandwidth in both directions.  Having an server with 10 Mbits/sec bandwith to the internet, for example, will only lead to audio and video issues with users.
...
Also, EC2 gives you a very small root partition.  Some of the directories in BigBlueButton (like logs files) can get large and use all the space on the root partition.  You'll want to move over the following three directories from BigBlueButton to the /mnt partition (which by default has 500G):
...
We (the core developers) have not isntaled BigBlueButton on any other version of Ubuntu.  It probably won't work.
...
If you are a brave soul and want to take a BigBlueButton server and manually install the equivalent components (and configuration files) onto a Windows server, it shoudld techniclly be possible.  However, beware, no one who has tried this has lived to post a success message to our mailing list :-).
...
We understand that administrators want to run multiple applications on a single server.  This makes sense when your running multiple web-based applications.  If a web application returns a web page in 350 milliseconds instead of 250 milliseconds due to resource contention, the perceptual difference to the user is minimal.  At worst, the user will think your web application is a little slow, but it still works.
...
Tunnelling is not a stable as connecting directly -- the BigBlueButton client must make about 10 HTTP requests/second to keep up with the sending/recieving of real-time content.  The user will have a more stable connection if their BigBlueButton client can connect directly through port 80.
...
The setting for picture quality of the webcams is in the [videoconf module](/overview/client-configuration.html#videoconf-module).
...
Since BigBlueButton must convert your PowerPoint document to PDF (using OpenOffice), none of the animations (visual or audio) will be converted.  What you'll see is a PDF page  corresponding to the final state of each slide.
