...
### Can more than one user use the whiteboard

Currently, only the presenter can use the whiteboad tools.  A moderator can make any user presenter (including themselves).

We do have multi-user whiteboard as an open enhancement request for implementation in the future (see [Extend whiteboard to enable multiple users at the same time](https://github.com/bigbluebutton/bigbluebutton/issues/2055)).
