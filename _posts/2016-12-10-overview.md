...
At this point, the moderator can begin typing text (or using a stenographer machine to inject text).
...
Clicking ‘Start Sharing’ will start sharing the selected region.  As with sharing full screen, a thumbnail version of what the remote students see will appear in the Screen Sharing Presenter’s Preview window.
...
<tr>
  ...
  <td>Setting `webcamsOnlyForModerator=true` will cause all webcams shared by viewers during this meeting to only appear for moderators.  This is useful when there is low bandwidth for remove students and they don't need to see each other's webcam.</td>
</tr>
<tr>
  <td>isBreakout</td><td>Optional</td><td>Boolean</td>
  <td>Setting `isBreakout=true` indicates that this is a breakout room.  In a breakout room, the create Breakout Room button is disabled.</td>
</tr>
</table>

### getMeetingInfo

With breakout rooms, `getMeetingInfo` will return additional information to let you determine which room is a breakout room.
...
If a meeting is a breakout room, then `getMeetingInfo` will return a link to the parent meeting.
...
We upgraded the version of FreeSWITCH to 1.6.15. This brings support for forward error correction (FEC) in WebRTC audio, which means viewers using Chrome (support for FEC is coming soon to FireFox) will have even better audio on wireless or low-bandwidth networks with packet loss.
...
This release builds upon red5 1.0.8-M13 which includes a number of bug fixes over the previous release we were using in BigBlueButton 1.0 (thanks to the [red5 team](http://red5.org/) for being so responsive to our requests).
