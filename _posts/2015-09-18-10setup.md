---
layout: page
title: "Setup Development Environment 1.0"
category: dev
...
This document describes how to setup a development environment using an existing BigBlueButton 1.0.0 server.  Once the environment is setup, you will be able to make custom changes to BigBlueButton source, compile the source, and replace the corresponding components on the server (such as updating the BigBlueButton client).
...
Before you can start developing on BigBlueButton, you must [install BigBlueButton 1.0.0](/install/install.html) and ensure it's working correctly. Make sure there were no errors during the installation and that you can join a session successfully.
