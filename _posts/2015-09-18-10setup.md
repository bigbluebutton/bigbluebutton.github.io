...
Welcome to the BigBlueButton Developer's Guide for BigBlueButton 1.0-beta (referred hereafter as 1.0).  

You'll need to have a working BigBlueButton 1.0 server before you can setup a development environment (see [installation steps](/1.0/10install.html)).
...
You don't need to understand everything about each component, but you do need to understand the [overall architecture](/1.0/10architecture.html) and how the components work together.
...
Before you can start developing on BigBlueButton, you must install BigBlueButton 1.0 (see [installation steps](/1.0/10install.html)) and ensure it's working correctly. Make sure there were no errors during the installation and that you can join a session successfully.
...
When you first clone the BigBlueButton git repository, git will place you, by default, on the `master` branch, which is the latest code for BigBlueButton.  The development for 1.0 (currently 1.0-beta) is occurring on `master`.
...
```
git checkout -b my-1.0-work upstream/master
```
