...
You can upgrade by running the `bbb-install-2.6.sh` script -- it will download and install the latest release of BigBlueButton 2.6 on top of your old 2.5 version.

## Upgrading from BigBlueButton 2.4

If you are upgrading BigBlueButton 2.4 we recommend you set up a new Ubuntu 20.04 server with BigBlueButton 2.6 and then copy over your existing recordings from the old server. Check https://docs.bigbluebutton.org/admin/customize.html#transfer-published-recordings-from-another-server for instructions how to copy the recordings.
