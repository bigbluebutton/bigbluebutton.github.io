...
(Note: BigBlueButton 1.1-beta is not yet released.  We'll be updating this URL when the release occurs.  To follow the release, subscribe to [bigbluebutton-dev](https://groups.google.com/forum/#!forum/bigbluebutton-dev) mailing list.)
...
Before following these instructions, we recommend you start with a dedicated Ubuntu 16.04 64-bit server that is not running prior applications.  Doing this will avoid many subtle conflicts (such as with underlying configuration files) that will cause errors in BigBlueButton installation and running.

If you already have a BigBlueButton 1.0 server running Ubuntu 14.04, we recommend **not** trying to upgrade your server.  Start with a clean Ubuntu 16.04 64-bit server, install BigBlueButton using the steps below, and then [copy over the recordings XXX](#reprocess-recordings-from-0.81) from the old BigBlueButton server to the new. 


# Before you install

The prerequisites for BigBlueButton 1.1 server are
...
If you intend to setup BigBlueButton for development, installing on a virtualized server works well. You probably don't need 500G free on the VM; 50G would be sufficient.  
...
XXX If you are installing BigBlueButton on EC2 (good for testing, but for production we recommend using a physical server for better performance), Amazon EC2 provides an external and internal IP address.  After you install BigBlueButton, complete the steps outlined [here](#audio-not-working).


## 2. Install key for BigBlueButton

The BigBlueButton packages are signed with a private key.  You need to provide apt-get the public key to validate the packages.  To add the key, do the following

~~~
$ wget http://ubuntu.bigbluebutton.org/repo/bigbluebutton.asc -O- | sudo apt-key add -
~~~

Next, add the BigBlueButton repository URL (note: BigBlueButton 1.1-beta is not yet released.  We'll be updating this URL when the release occurs.  To follow the release, subscribe to [bigbluebutton-dev](https://groups.google.com/forum/#!forum/bigbluebutton-dev) mailing list.)
