...
BigBlueButton is an open source web conferencing system for online learning.  BigBlueButton enables instructors to provide remote students a high-quality learning experience.  For an overview what is new in this release, see [overview](/1.1/overview.html).
...
We recommend **not** trying to upgrade an earlier version of BigBlueButton, but instead start with a clean Ubuntu 16.04 64-bit server, install BigBlueButton using the steps below, and then [transfer over existing recordings](##transfer-existing-published-recordings).  (Earlier versions ran on Ubuntu 14.04 and 12.04, so you really do want to start with a clean server.)

These instructions will walk through in detail all the steps.  We have also made a video walking through the steps to install BigBlueButton 1.1 on an new server, assigning a hostname, and configuring it with SSL certificate from letsencrypt.  Click the image below to view the video.

<a href="http://www.youtube.com/watch?feature=player_embedded&v=6Saztv2WlKI
" target="_blank"><img src="http://img.youtube.com/vi/6Saztv2WlKI/0.jpg" 
alt="Installation Video Walkthrough" width="240" height="180" border="10" /></a>
...
If you don't yet have a SSL certificate and your server is on the internet, we provide steps to use Let's Encrypt to obtain a free renewable SSL certificate (exprires after 90 days).  See [setup using Let's Encrypt](#using-lets-encrypt).
