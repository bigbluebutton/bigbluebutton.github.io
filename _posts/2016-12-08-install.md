...
If you are not running BigBlueButton in production, installing on a virtualized machine (VM) works well. You probably don't need 500G free on the VM; 50G would be sufficient.  
...
We have designed, developed, installed, and tested BigBlueButton 1.1 on Ubuntu 16.04 64-bit (Xenial Xerus).  We have neither tested nor install BigBlueButton 1.1 on any other version of Ubuntu.  If you try this, it probably won't work.
...
At this point, you have a BigBlueButton server ready for testing.  You can run the BigBlueButton configuration utility `bbb-conf` to check your configuration for any errors

~~~
$ bbb-conf --check
bbb-conf --check

BigBlueButton Server 1.1.0-beta (387)
...
The output for your `bbb-conf --check` will be different.  If your server has a hostname, you can use `bbb-conf` to configure BigBlueButton to listen for incoming requests on the hostname.  For example, to configure BigBlueButton to use the hostname 'bbb.myserver.com' 
...
To restart the BigBlueButton, enter the following commands:
...
BigBlueButton Server 1.1.0-beta (311)
