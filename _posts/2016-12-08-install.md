...
---
   ...
   *Note:* BigBlueButton 1.1-beta is not yet released.  This page is still DRAFT.  To follow news on the upcoming 1.1-beta release, subscribe to [bigbluebutton-dev](https://groups.google.com/forum/#!forum/bigbluebutton-dev) mailing list.)

Welcome to the installation guide for BigBlueButton 1.1-beta (referred hereafter as BigBlueButton 1.1).

BigBlueButton is an open source web conferencing system for online learning.  For an overview what is new in this release, see [overview](/1.1/overview.html).

Before following these instructions, we recommend you start with a clean Ubuntu 16.04 64-bit server that is not running other web applications.  Doing this will avoid many subtle conflicts (such as with underlying configuration files) that will cause errors in BigBlueButton installation and running.

We recommend **not** trying to upgrade your BigBlueButton 1.0 server (which ran on Ubuntu 14.04).  Start with a clean Ubuntu 16.04 64-bit server, install BigBlueButton using the steps below, and then [transfer over existing recordings](##transfer-existing-published-recordings). 


# Before you install

The prerequisites for installing BigBlueButton 1.1 are
...
Why do we recommend a bare metal server?  BigBlueButton uses FreeSWITCH for processing the incoming real-time packets for audio, and FreeSWITCH works best in a non-virtualized environment (see [FreeSWITCH recommended configurations](https://freeswitch.org/confluence/display/FREESWITCH/Performance+Testing+and+Configurations#PerformanceTestingandConfigurations-RecommendedConfigurations)).

Installing BigBlueButton on a virtualized machine (VM) works well for testing and development.  Also, you probably don't need 500G free for testing and development (25G would be more than sufficient).  

For users connecting to BigBlueButton we recommend (a minimum of) 1.0 Mbits/sec download speed and 0.5 Mbits/sec upload speed.
...
and logout and log back into your SSH session (this will reload your configuration).  Next, run `cat /etc/default/locale` and verify you see only the single line `LANG="en_US.UTF-8"`.  Note: if you see an additional line `LC_ALL=en_US.UTF-8`, then remove the setting for `LC_ALL` before continuing.

Check that your server is running 64-bit version of Ubuntu 16.04.
...
Check your version of Ubuntu is Ubuntu 16.04.  
...
We have designed, developed, installed, and tested BigBlueButton 1.1 on Ubuntu 16.04 64-bit (Xenial Xerus).  Why the focus on one distribution?  It's a choice of quality over quantity.  We figured long ago it would be better for the community to have solid installtion for a specific version of Linux rather than have many installations that don't work well.  


# Installing BigBlueButton 1.1-beta

A few words about the installation steps below.  Finish each step before proceeding to the next.  If you encounter an error at any step, STOP and double-check you've entered the proper commands.  Don't continue onto the next step as you'll only compound the errors.  If you can't resolve an error using the documentation below (or using Google to search for the error messages), then post a description of the error to the [bigbluebutton setup](https://groups.google.com/forum/#!forum/bigbluebutton-setup) mailing list and the community will help.  
...
Login to your server via SSH.  You need to have an account that can execute commands as root (via sudo).  First, ensure that you have `xenail multiverse` in your `sources.list`.  Do the following.
...
Before proceeding further, upgrade all packages on the server to ensure everything is up-to-date.
...
Note: If you are installing BigBlueButton on EC2 (good for testing, but for production we recommend using a physical server for better performance), Amazon EC2 provides an external and internal IP address.  After you install BigBlueButton, complete the steps outlined [here](#audio-not-working).


## 2. Install apt-get key and repository for BigBlueButton

You need to provide the BigBlueButton apt-get public key to validate the packages.  To add the key, do the following

~~~
$ wget http://ubuntu.bigbluebutton.org/repo/bigbluebutton.asc -O- | sudo apt-key add -
~~~

Next, add the BigBlueButton repository URL (note: BigBlueButton 1.1-beta is not yet released.  We'll be updating this URL when the release occurs and replacing 'XXX' with the final location.  To follow the release, subscribe to [bigbluebutton-dev](https://groups.google.com/forum/#!forum/bigbluebutton-dev) mailing list.)

~~~
$ echo "deb http://ubuntu.bigbluebutton.org/XXX/ bigbluebutton-xenial main" | sudo tee /etc/apt/sources.list.d/bigbluebutton.list
~~~
...
After the install, you should a BigBlueButton server ready for testing.  You can run the BigBlueButton configuration utility `bbb-conf` to check your setup for errors.
...
The items under `Potential problems described below` may indiciate configuration errors or installation errors.  

If your server has a hostname (some browsers require a hostname for WebRTC and you'll definitely need it for configuring SSL), use `bbb-conf` to assign the hostname to BigBlueButton's configuration files.  For example, to configure BigBlueButton to use the hostname 'bbb.myserver.com' 

~~~
$ bbb-conf --setip bbb.myserver.com
~~~

At this point, you can use an [existing front-end](http://bigbluebutton.org/open-source-integrations/) integration or setup your own integration with the [API](/dev/localization.html).  Either way, the integrations or API calls will need the URL for the BigBlueButton server and its shared secret.  You can get both using the following command:

~~~
$ bbb-conf --secret

       URL: http://bbb.myserver.com/bigbluebutton/
    Secret: a7007506f1efffa497922fc34e3184dc
~~~

For more options, see [bbb-conf](/install/bbb-conf.html).

BigBlueButton also comes with a number of API demos to let you quickly test the server.  To install the demos, go to the next step. 

## 4. Install API Demos (optional)

In addition to using a [3rd party integration](http://bigbluebutton.org/open-source-integrations/)), you can install a set of API demos that let you quickly try out BigBlueButton.
...
BigBlueButton has a self-check utility that can help diagnose networking and configuration issues that may prevent a user from launching BigBlueButton.  

~~~
$ sudo apt-get install bbb-check
~~~

This is the same self-check page you see at [check](http://demo.bigbluebutton.org/check).
...
BigBlueButton will automatically launch on startup.  You can restart your server at any time using the commandd

~~~
$ sudo bbb-conf --restart
~~~

The bbb-conf check scans some of the log files for error messages.  If these message exist from a previous execution, you can clean out the log files and restart at the same time using the commands.
...
As before, the `--check` option will grep through the log files looking for errors.
...
Notice that ```sudo bbb-conf --check``` warns you the API demos installed, which would enable anyone with access to the server to launch sessions.
...
If already have a BigBlueButton 0.81 server, these instructions will **not** upgrade your server.  You need to install on a clean installation of 1.1 on Ubuntu 16.04 64-bit and follow the steps previous in this document.


# Configuring SSL on your BigBlueButton

You'll want to add SSL support to nginx to loading web pages via HTTPS.  Also, as of Chrome 47, Chrome users will be unable to share their microphone via WebRTC unless BigBlueButton is loaded via HTTPS.  
...
Next, to generate a certificate on your server, use the following command (replace yourserver.example.com with your own dns name):
...
~~~
  /etc/letsencrypt/live/yourserver.example.com/fullchain.pem;
  /etc/letsencrypt/live/yourserver.example.com/privkey.pem;
~~~
...
To finish the configuration, continue with the steps at [Configure FreeSWITCH to user WebRTC](#configure-freeswitch-for-using-webrtc).
