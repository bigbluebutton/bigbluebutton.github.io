...
Note: during install, will see an error "Failure to download extra data files" with ttf-mscorefonts-installer.  This is a [known issue](https://bugs.launchpad.net/ubuntu/+source/aptitude/+bug/1543280) with Ubuntu 16.04.  You can ignore this error.

After the install, your BigBlueButton server is ready for testing.  First, run the BigBlueButton configuration utility `bbb-conf --check` to check your setup for errors.  You will see output as follows (with a different IP address).
...
If your server has a hostname (some browsers require a hostname for WebRTC and you'll definitely need it for configuring SSL), use `bbb-conf --setip` to assign the hostname to BigBlueButton's configuration files.  For example, to configure BigBlueButton to use the hostname 'bbb.myserver.com', type

~~~
$ bbb-conf --setip bbb.myserver.com
~~~

At this point, you can use an [existing front-end](http://bigbluebutton.org/open-source-integrations/) integration or setup your own integration with the [API](/dev/localization.html).  To configure a 3rd party front-end or use the API in your programs, will need the BigBlueButton server URL and its shared secret.  You can get both using the following command:
...
BigBlueButton also comes with a set of API demos to let you quickly test the server.  To install these demos, go to the next step. 
...
BigBlueButton's self-check utility that can help diagnose networking and configuration issues that may prevent a user from launching BigBlueButton.  
...
You can restart your BigBlueButton server at any time using the command
...
It's good do a `bbb-conf --check` again after restart to run the self-checks to detect any issues.  The `bbb-conf --check` scans some of the log files for error messages.  So, if you want to clear out the log files and star clean, use
...
Edit the file `/opt/freeswitch/conf/sip_profiles/external.xml` and look for a line containing "ws-binding". Add a change it to the following
