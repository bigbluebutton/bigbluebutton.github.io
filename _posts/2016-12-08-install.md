...
The prerequisites for installing BigBlueButton 1.1 are
  ...
  * TCP port 7443 if you intend to configure SSL (recommended), otherwise port 5066
  * UDP ports 16384 - 32768 are accessible
  * Port 80 is **not** in use by another application
  * 500G of free disk space (or more) for recordings
  * 100 Mbits/sec bandwidth (symmetrical)
  * Dedicated (bare metal) hardware (not viritualized)
...
However, if your are doing developmet and testing, installing BigBlueButton on a virtualized machine (VM) works fine.  Also, you probably don't need 500G free for testing and development (25G would be more than sufficient).  If you are installing on EC2, for example, visit section [audio not working](#audio-not-working) to adjust FreeSWITCH to work with internal/external IP addresses.
...
Can BigBlueButton 1.1 install on other versions of Ubuntu?  Likely not.  We have designed, developed, installed, and tested BigBlueButton 1.1 on a single distriubtion of Ubuntu: 16.04 64-bit (Xenial Xerus).  That's what we recommend you use.

Why the focus on a single distribution?  It's a choice of quality over quantity.  We figured long ago it would be better for the BigBlueButton community to have solid installtion for a specific version of Linux that works really, really well than to have many installations that don't work well.  We do get occasional request for a [CentOS](/support/faq.html#centos).


# Installing BigBlueButton 1.1-beta

Before starting the installation, a few words of wisdom.  


1. Check the server matches the [minimum specifications](#before-you-install).  For example, does it have enough memory?  You can still install with less than 4G of memory, but your going to have problems running BigBlueButton.  Is the locale set to `en_US.UTF-8`?  If not, your going to encounter errors when trying to install.  

2. Don't skip over errors on installation.  Continuing to the next step will only compound the errors.  Instead, if you encounter an error, STOP and double-check you've entered the proper commands.  Next, check the [troubleshooting](#troubleshooting-installation) section for similar errors.  Google the error as well.

3. If you can't resolve an error using the documentation below (or using Google to search for the error messages), then post a description of the error to the [bigbluebutton setup](https://groups.google.com/forum/#!forum/bigbluebutton-setup) mailing list and the community will help.  

4. You are going to want to configure SSL your BigBlueButton server.  While you don't need to do this immediately, its good for security and WebRTC support to have your BigBlueButton server configured with SSL.  BigBlueButton supports WebRTC audio on FireFox and Chrome -- which gives you high-quality low-latency audio.  FireFox will use WebRTC from sites loaded via HTTP (no SSL setup).  However, as of Chrome 47, Chrome users will be unable to share their microphone via WebRTC unless BigBlueButton is loaded via HTTPS.  Setup of SSL requires your server have a domain name (you'll need to purchase a domain name if you don't already have one) and obtain SSL certificate for that domain name.  There is *lots* of documentation on the web for how to do both.  You'll see steps below on how to configure your BigBlueButton server with SSL.

Need some commercial support or hosting?  There are a number of [commercial companies](http://bigbluebutton.org/support) that contribute towards the development of BigBlueButton.  If you want a bit of help (and want to support the development of BigBlueButton), check them out.
...
After the install finishes, run the BigBlueButton configuration utility `bbb-conf --check` to check your setup for errors.  This utility runs a few dozen checks on your server and tries to find any potential errors.

You should see output similar to the following (with a different IP address).
...
The items under `Potential problems` may indiciate configuration errors or installation errors. 

At this point, your BigBlueButton server is listening on an IP address. If you have a hostname already setup for the server, you can update all of BigBlueButton's configuration files to use the hostname with a single command.  

~~~
$ bbb-conf --setip <hostname>
~~~

For example, if your hostname was bbb.myserver.com, you would enter

~~~
$ bbb-conf --setip bbb.myserver.com
~~~

At this point, you have BigBlueButton server listening to an IP address (or hostname) and resonding to API requests.  However, if you tried to login from your server's default welcome page right now, you would get an error `HTTP Status 404 - /demo/demo1.jsp`.  

Why?  This page requires the API demos if you want to test out the server.  The instructions on ow to install (and uninstall) the API demos are in the next step.

However, if you already have an [existing front-end](http://bigbluebutton.org/open-source-integrations/), such as Moodle, and you want to connect it with your new BigBlueButton server, you need to configure the integration server's hostname and its shared secret.  You can get both of these using the following [bbb-conf](/install/bbb-conf.html) command:
...
Finally, if you are a developer and want to integrate BigBlueButton with your web-based application, you probably want to install the API examples (see next step) and check out the [BigBlueButton API](http://docs.bigbluebutton.org/dev/api.html) documentation for how to integrate BigBlueButton with your application.
...
Once installed, you'll be able to enter your name on the home page and click 'Join'.  This will join you into the a demo meeting.  You can also check out the other examples by clicking the `API exmples` link on the home page.  These are the same API demos installed at [http://demo.bigbluebutton.org](http://demo.bigbluebutton.org).
...
It's good do a `bbb-conf --check` after restart to run the self-checks to detect any issues.  The `bbb-conf --check` scans some of the log files for error messages.  If you restart those previous log files still exist, so clear out the log files and run the check you can use the `--clean` option, as in
...
Here is a sample output from running `sudo bbb-conf --check` on demo.bigbluebutton.org.
...
If you had installed the package `bbb-check` in [step 5](#install-client-self-check-optional), you need to make the same cahnge to `/var/www/bigbluebutton/check/resources/lib/bbb_webrtc_bridge_sip.js`:
