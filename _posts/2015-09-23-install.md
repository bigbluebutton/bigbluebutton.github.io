...
Welcome to the installation guide for BigBlueButton 1.0.  BigBlueButton is an open source web conferencing system for on-line learning.  For an overview what new in this release, see [overview](/overview/overview.html).
...
Next,before you upgrade your BigBlueButton 1.0 server, you need to ensure you have libssl1.0.2 installed to enable FreeSWITCH to support ECDSA keys (see [more information]( https://developers.google.com/web/updates/2016/06/webrtc-ecdsa?hl=en)).  To check, do the following command
...
If you only see `libssl1.0.0`, then add the following PPA

~~~
$ sudo add-apt-repository -y ppa:ondrej/php
~~~

Your ready to upgrade.  To upgrade an earlier version of BigBlueButton 0.9/1.0, do the following:
