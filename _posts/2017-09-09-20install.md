...
* Assign a hostname (recommended to set up SSL)
...
If you are setting up BigBlueButton for local development on your workstation, you can relax the server requirements slightly as you'll be the only one using the server.  You should be able to run BigBlueButton with:
...
Taking a moment to do these checks will significantly reduce the chances you'll hit a problem during installation.
...
Next, check that your server has (at least) 4G of memory using the command `free -h`.  Here's the output from one of our test servers.
...
Next, check that the server has Ubuntu 16.04.  
...
Before you upgrade, if you've done any custom changes to BigBlueButton through modifying configuration files, you need to back up these changes first so you can re-apply them after upgrading.
...
Next, use `bbb-conf --setip` to re-assign your server's IP address or hostname to BigBlueButton's configuration files.
...
If you installed a previous version of BigBlueButton 2.0 and then made custom changes, you need to backup your changes first.  After you backup your changes, you can then use [bbb-install.sh](https://github.com/bigbluebutton/bbb-install) to upgrade or follow the steps below.  After the upgrade, reapply your changes.
...
Note: If you're using [bbb-install.sh](https://github.com/bigbluebutton/bbb-install) to upgrade from BigBlueButton 2.0 you don't need to purge `bbb-client` and the Kurento packages as the upgrade script will detect you're upgrading from BigBlueButton 2.0 and automatically do it for you.

# Installation

Ready to install?  Here's a quick pre-install checklist:

1. You have an Ubuntu 16.04 64-bit server that meets the [minimum specifications](#before-you-install).
...
4. You have a valid SSL certificate for the hostname (or intend to obtain one with Let's Encrypt, which is covered in these documents)

If you are a developer setting up BigBlueButton for development or testing on a local VM with and intend to use FireFox, you can skip the above steps (2), (3), and (4).  

At this point, you are ready to install.  Grab a cup of your favourite beverage and let's begin!

## 1. Update your server

First, make sure your server is up-to-date with the latest packages and security updates.   
...
Don't worry if your hostname in the URL is different from the above. What's important is to see an uncommented link that contains `multiverse`.  If you don't, run the following command to add the multiverse repository to your `/etc/apt/sources.list` file.

```bash
$ echo "deb http://archive.ubuntu.com/ubuntu/ xenial multiverse" | sudo tee -a /etc/apt/sources.list
```

If you are a developer installing BigBlueButton on a VM for testing and development, some of BigBlueButton's components such as Tomcat, need a source of entropy when starting up.  In a VM the available entropy can run low Tomcat and can block for a long period of time (sometimes minutes) before finishing its statup.  To give the VM lots of entropy, install a package called `haveged` (a simple entropy daemon):
...
BigBlueButton HTML5 client uses MongoDB, a very efficent database used to synchronize the state of the clients.  To install MongoDB, do the following 
...
Note 2: If the installation exits with an error before finishing, double-check the steps in [Before you install](#before-you-install).  If you find and resolve any configuration errors, you can attempt to finish the installation using the command `sudo apt-get install -f`.
...
Notice that `sudo bbb-conf --check` warns you the API demos are installed, which enables anyone with access to the server to launch a session (see [removing API demos](/2.2/customize.html#remove-the-api-demos)).
...
* [https://demo.bigbluebutton.org](https://demo.bigbluebutton.org/) - Runs the latest stable version of BigBlueButton with the Greenlight front-end
...
For any production of a BigBlueButton server, you need to assign it a hostname.  
...
At this point, you have the BigBlueButton server listening to an IP address (or hostname) and responding to API requests.  However, if you tried to login from the server's default page with a browser, you would get an error `HTTP Status 404 - /demo/demo1.jsp`.  
...
Once you have a domain name and have configured it with a DNS host, add an A record pointing to your server. You can then use the [bbb-conf setup command](/install/bbb-conf.html) to configure BigBlueButton to use that domain name, for example:
...
Depending on your certificate authority (CA), you should now have two or more files, as follows:
...
At the time of writing, the configuration shown on this page should achieve an "A" ranking in the SSL Labs test page.

## Using Let's Encrypt

If you have a domain name assigned to your BigBlueButton server (i.e. bigbluebutton.example.com) and the server is on the Internet, then you can use Let's Encrypt to obtain a free SSL certificate.
...
The Let's Encrypt certificates are good for 90 days and can be automatically renewed.  To automatically request a renewal once a week, edit the crontab file for root.
