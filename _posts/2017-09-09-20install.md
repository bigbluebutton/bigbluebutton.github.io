...
You can [upgrade directly from a BigBlueButton 1.1 server](#upgrade-from-bigbluebuttton-11-to-20), or install BigBlueButton 2.0 on a new server via two options:
  ...
  * [Install via script](#install-via-script) if you server has a single IP address and is not behind NAT
...
To make it easy to install BigBlueButton 2.0, we've creatd an installation script that automates the installation/upgrade on some servers.

If you intend to setup and administrate a BigBlueButton server, we recommend going through the [step-by-step](#step-by-step-install) instll steps.  Alternatively, if you are familiar with the steps, or if you just want to quickly setup a BigBlueButton server to test it out, the install script will get you going in a few minutes.

See documentation for [bbb-install.sh](https://github.com/bigbluebutton/bbb-install).
