...
We recommend installing BigBlueButton with a 'clean' and dedicated Ubuntu 18.04 64-bit server with no prior software installed. If you want to upgrade from BigBlueButton 2.4, you can upgrade your existing server using the `bbb-install-2.5.sh` script and passing `bionic-250` instead of `bionic-240` as a repository. If you want to upgrade from an earlier version of BigBlueButton like 2.2, we recommend setting up a clean server for BigBlueButton 2.5 and, after setup, [migrate over your existing recordings](/admin/customize.html#transfer-published-recordings-from-another-server).
...
You can upgrade by re-running the `bbb-install-2.5.sh` script again -- it will download and install the latest release of BigBlueButton 2.5.

## Upgrading from BigBlueButton 2.4

if you are upgrading BigBlueButton 2.4, you can simply re-run the bbb-install command you used for 2.4 but replacing the repository `bionic-240` with `bionic-250` and `bbb-install.sh` with `bbb-install-2.5.sh`.
