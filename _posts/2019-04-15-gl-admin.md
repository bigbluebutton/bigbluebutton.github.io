...
### Modify user in command line

For example, to reset the password of a user or if there were errors when creating the user.

`docker exec -it greenlight-v2 bash`
`bundle exec rails c`
`User.find_by(email: "<email of user to reset>").update_attribute(:password,"<new password>")`
It shows only the first result.
Attributes can be for example **id, name, username, email, password** and much more.

Example:
`User.find_by(username: "Example2").update_attribute(:password,"12345678")`
