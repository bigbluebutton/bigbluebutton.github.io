...
In some scenarios your BigBlueButton server is tied to a tier web-application, and when a meeting
is ended you want to get informed about that event and do your own data processing. In that case, you
need to use the `meta` parameter named `endCallbackUrl`. It needs to be added in the create call like the line below :

* http&#58;//yourserver.com/bigbluebutton/api/create?name=Test&meetingID=test01&***meta_endCallbackUrl=https://www.company.com/backend/meeting_end***&checksum=1234

BigBlueButton will store this URL until the meeting is ended. When the meeting is ended, this URL
will be called with an additional parameter to inform about recordings availablity in the future 

https://www.company.com/backend/meeting_end&***recordingmarks=(true false)***

If recording was initiated at least once the value of `recordingmarks` will be set to true.
In the other case its value will be set to `false`.
