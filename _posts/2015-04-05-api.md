...
The API is implemented with developer in mind. To make an API call to your BigBlueButton server, your application makes HTTP requests to the servers API endpoint (usually the server's hostname followed by `/bigbluebutton/api`).  All API calls must incude checksum computed with a secret shared with the BigBlueButton server.
...
| sequence        | Required(Breakout Room)    | Number  | The sequence number of the brekaout room. |
...
When designing a front-end for BigBlueButton, it is sometimes helpful to know as soon as a meeting has finished to update the front-end.  For example, whe the meeting ends the front-end could hid the 'join' button.

To know when the meeting ends, you can pass a call-back URL using the meta-parameter `meta_endCallbackUrl` on the `create` command.  If this parameter exists, then the BigBlueButton server will do a HTTP request to the given URL when the meeting ends.

Because you can specify the URL, you can include additional information such as the `meetingID`.  Here is an example callback URL:

~~~
https://front-end.example.com/callback?meetingID=test01
~~~

To pass this parameter along with the `create` command, you would add `&meta_endCallbackUrl=https%3A%2F%2Ffront-end.example.com%2Fcallback%3FmeetingID%3Dtest01` to the `create` command parameters.

BigBlueButton will store this URL until the meeting is ended. When the meeting ends, it will call this URL and add an additional parameter: `recordingmarks` which will be either `true` or `false`.  It will be `true` if the meeting was set to be recorded **and** a moderator clicked the Start/Stop Record button (which places recording marks in the events).  Given the sample callback URL above, here's the final URL that BigBlueButton will call when the meeting ends and there were recording marks.

~~~
https://front-end.example.com/callback?meetingID=test01&recordingmarks=true
~~~
