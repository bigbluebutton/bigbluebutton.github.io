...
| attendeePW  | Optional             | String  | The password that the [join](#join) URL can later provide as its `password` parameter to indicate the user will join as a viewer.  If no `attendeePW` is provided, the `create` call will return a randomally generated `attendeePW` password for the meeting. |
| moderatorPW | Optional             | String  | The password that will [join](#join) URL can later provide as its `password` parameter to indicate the user will as a moderator.  if no `moderatorPW` is provided, `create` will return a randomly generated `moderatorPW` password for the meeting. |
