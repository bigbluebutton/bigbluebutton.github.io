...
| duration       | Optional                | Number    | The maximum length (in minutes) for the meeting.  BigBlueButton begins tracking the length of a meeting when the first person joins.  If duration is a non-zero value, then when the length of the meeting exceeds the duration value the server will immediately end the meeting (any users still in the meeting are kicked and the meeting is cleared from memory).  If no duration parameter is passed (or its value is 0), then BigBlueButton will end the meeting when either (a) the last person leaves (it takes about two minutes for the meeting to be cleared from memory) or the server receives an [end](#end) API request with the associated meetingID (the meeting is immediately cleared from memory). |
...
To help you create valid API calls, use [API Mate](http://mconf.github.io/api-mate/) which lets you interactively create API calls and test them against a public BigBlueButton server
