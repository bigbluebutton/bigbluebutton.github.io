...
This document gives you an overview of the features of BigBlueButton.   Want to watch a quick overview video of how easy it is to join as a user?  We uploaded [![BigBlueButton 2.0 Overview](https://www.youtube.com/watch?v=dhoIZ9i-JYE/0.jpg)](https://www.youtube.com/watch?v=GeJbK1lvl2I "Viewer Overview") to YouTube.
...
In BigBlueButton version 1.1, if you wanted to get a snapshot of all meeting and user activity on your server, you needed to do a `getMeetings` API call and then itterate through each meeting and do a `getMeetingsInfo` API call. 
...
The responses to the feedback are sent to the BigBlueButton server via the [client logging](http://192.168.0.130:4000/overview/overview.html#client-logging)
...
This release adds an option in the lock settings to lock the viewer's webcaqms so they are only visible to moderators.
