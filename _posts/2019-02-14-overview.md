...
BigBlueButton offers real-time sharing of audio, video, presentations, and the user's screen.  To engage remote students, BigBlueButton offers interactive chat (public and private), multi-user whiteboard, shared notes, emojis, polling, and breakout rooms.  BigBlueButton can record all content for later playback.

While we focus on the following use cases for online learning
  * Vitual office hours/tutoring
  * Flipped classroom (recording content ahead of class)
  * Student collaboration
  * Full online classes

 BigBlueButton can be use anywhere you want a web conferencing system.  BigBlueButton offers a modern HTML5-based interface that works across desktop, laptop, chromebook, and mobile (iOS 11+ and Android 6.0+) devices.

BigBlueButton is [open source](https://github.com/bigbluebutton/bigbluebutton), is backed by a [supportive community](https://bigbluebutton.org/support/), and can be [installed about 15 minutes](https://github.com/bigbluebutton/bbb-install).

# Features

In a BigBlueButton session there are only two types of users: a viewer or moderator.

A viewer (typically the student) can chat, send/receive audio and video, respond to polls, and display an emoji (such as raise hand).  A session can have multiple viewers.

A moderator (typically the instructor) has all the capabilities of a viewer plus the ability to mute/unmute other viewers, lock down viewers (i.e. restrict them from doing private chat), and assign anyone (including themselves) the role of presenter.  A session can have multiple moderators.

The presenter controls the presentation area. This means they can upload slides, annotate the current slide with the whiteboard controls, enable/disable multi-user whiteboard, start a poll, and share their screen for all to see.  A moderator can make anyone the presenter (including themselves).  Only one person at a time can be presenter.
...
Want to try BigBlueButton yourself?  Visit our demo server at [https://demo.bigbluebutton.org/](https://demo.bigbluebutton.org/). You can sign in with your Google or Twitter account (no password requred) and start creating meetings.

Want to setup your own BigBlueButton server (or upgrade from the previous version)?  See the [install](/2.2/install.html) steps.
