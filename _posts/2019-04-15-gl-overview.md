...
Generates an access code that users must enter before they are allowed to join the room. 
An access code can be randomly generated by clicking the dice icon the left, and can be removed by clicking the garbage icon on the right.
...
Gives all users moderator priveleges in BigBlueButton when they join the meeting.
