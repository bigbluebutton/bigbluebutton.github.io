...
We'll refer to this value as `sharedSecret`. When you first [install BigBlueButton](2.4/install.html) on a server, the packaging scripts create a random 32 character `sharedSecret`. You can also change the `sharedSecret` at anytime using the command `bbb-conf --setsecret`.
