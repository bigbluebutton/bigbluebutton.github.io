...
With BigBlueButton 2.4-rc-4 (or later) you can switch the way full audio (microphone) is handled on the backend. By default we use SipJS but we are working on connecting to Kurento (and later MediaSoup) and then to FreeSWITCH rather than directly to FreeSWITCH. The result is a significant reduction of CPU usage. This audio setup currently does not support echo test, so it needs to be disabled when using.
...
After a restart of BigBlueButton (`sudo bbb-conf --restart`) you can try out this audio join method.
Reverting to the default options can be achieved by removing the override sections (and passed API parameters) and restart of BigBlueButton.
