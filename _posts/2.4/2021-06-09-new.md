...
This release also contains initial support for using [Mediasoup](https://mediasoup.org/) instead of Kurento for handling WebRTC video streams (webcams and screenshare).  See these [steps](https://github.com/bigbluebutton/bbb-webrtc-sfu/wiki/Back-of-the-envelope-calculations:-mediasoup) for enabling mediasoup on your setup of 2.4-rc-2 (or later).  For analysis on Medisoup vs. Kurento, see [BigBlueButton World - BigBlueButton's Media Stack and the Road Ahead](https://youtu.be/SBO5iWLs0KE).
