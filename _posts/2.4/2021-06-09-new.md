...
Moderators (teachers) are now able to monitor user participation and engagement throughout the session.  The Learning Dashboard gives moderators and additional live view (updated every 10 seconds) of user data that includes
...
Teachers use this dashboard to track student engagement during the session, answering questions such as 

 - Which students are participating in the session (and which are not)?
 - Based on the poll results, which students are learning the content (and which are not)?
