...
At some point, we'll have an official BigBlueButton Docker image at Docker Hub.  For now, we invite you to the image for BigBlueButton youself and give us feedback.


# Before you run BigBlueButton in docker

First, if you have not already done so, you need to install [Docker](https://docs.docker.com/engine/installation/) on your computer.   Next, checkout the source files from [BigBlueButton Docker](https://github.com/bigbluebutton/docker).

If you have a local `apt-get` caching server (such as using `apt-cacher-ng`), then edit the Docker file, ununcomment the folling line and change 192.168.0.130 to match the hostname/IP address of your proxy server.
...
bigbluebutton/docker      latest   ca5ab0317d11     18 hours ago        3.11 GB
~~~
...
~~~
docker run -p 80:80/tcp -p 1935:1935/tcp -p 5066:5066/tcp -p 32730-32768:32730-32768/udp -p 2202:2202 --cap-add=NET_ADMIN bigbluebutton/docker -h 192.168.0.130
~~~
